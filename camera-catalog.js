...
(function(){
  ...
  if(typeof originalRender==='function'){
    window.render=function(){const result=originalRender.apply(this,arguments);refreshExactProductCount();return result;};
  }

  const state={search:'',brand:'',category:'',stock:'',sort:'name:asc'};
  let rowsData=[];
  let panel=null;
  ...
  function filtered(){
    ...
    let list=rowsData.filter(p=>{
      const hay=norm([p.name,p.brand,p.model,p.category,p.description].join(' '));
      if(q&&!hay.includes(q))return false;
      if(state.brand&&norm(p.brand)!==norm(state.brand))return false;
      if(state.category&&norm(p.category)!==norm(state.category))return false;
      if(state.stock==='in'&&(Number(p.stock)||0)<=0)return false;
      if(state.stock==='out'&&(Number(p.stock)||0)>0)return false;
      return true;
    });
    const [field,dirText]=(state.sort||'name:asc').split(':');
    list.sort((a,b)=>compare(a,b,field,dirText==='desc'?-1:1));
    return list;
  }
  ...
  function renderRows(){
    ...
    if(count)count.textContent=`${list.length} ürün gösteriliyor / ${rowsData.length} toplam`;
  ...
  async function loadFilterData(){
    if(typeof client==='undefined'||!client||typeof user==='undefined'||!user)return;
    try{
      const r=await client.from('products').select('*').order('name',{ascending:true});
      if(r.error)throw r.error;
      rowsData=r.data||[];
      buildPanel();
      renderRows();
    }catch(e){console.error('Ürün filtreleri:',e)}
  }
  ...
  function buildPanel(){
    ...
    panel.id='productFilters';
    panel.style.cssText='display:grid;grid-template-columns:minmax(220px,2fr) repeat(4,minmax(130px,1fr)) auto;gap:8px;align-items:end;margin:14px 0;padding:12px;background:#f8fafc;border:1px solid #e5e7eb;border-radius:10px';
    panel.innerHTML=`<div><label>Ürün ara</label><input id="productSearch" type="search" placeholder="Ürün, marka, model, kategori..." autocomplete="off"></div><div><label>Marka</label><select id="productBrandFilter"><option value="">Tüm markalar</option></select></div><div><label>Kategori</label><select id="productCategoryFilter"><option value="">Tüm kategoriler</option></select></div><div><label>Stok</label><select id="productStockFilter"><option value="">Tüm stoklar</option><option value="in">Stokta var</option><option value="out">Stokta yok</option></select></div><div><label>Sıralama</label><select id="productSort"><option value="name:asc">Ürün A → Z</option><option value="name:desc">Ürün Z → A</option><option value="brand:asc">Marka A → Z</option><option value="brand:desc">Marka Z → A</option><option value="category:asc">Kategori A → Z</option><option value="category:desc">Kategori Z → A</option><option value="model:asc">Model A → Z</option><option value="model:desc">Model Z → A</option><option value="price:asc">Fiyat düşük → yüksek</option><option value="price:desc">Fiyat yüksek → düşük</option><option value="stock:asc">Stok az → çok</option><option value="stock:desc">Stok çok → az</option></select></div><div><button id="productFilterReset" class="light" type="button">Temizle</button></div><div id="productFilterCount" class="muted" style="grid-column:1/-1">0 ürün gösteriliyor</div>`;
    ...
    [...new Set(rowsData.map(p=>String(p.brand||'').trim()).filter(Boolean))].sort((a,b)=>norm(a).localeCompare(norm(b),'tr')).forEach(v=>brand.insertAdjacentHTML('beforeend',`<option value="${esc(v)}">${esc(v)}</option>`));
    [...new Set(rowsData.map(p=>String(p.category||'').trim()).filter(Boolean))].sort((a,b)=>norm(a).localeCompare(norm(b),'tr')).forEach(v=>category.insertAdjacentHTML('beforeend',`<option value="${esc(v)}">${esc(v)}</option>`));

    if(listenersReady)return;
    const apply=()=>{state.search=document.getElementById('productSearch').value;state.brand=brand.value;state.category=category.value;state.stock=document.getElementById('productStockFilter').value;state.sort=document.getElementById('productSort').value;renderRows();};
    ['productSearch','productBrandFilter','productCategoryFilter','productStockFilter','productSort'].forEach(id=>document.getElementById(id).addEventListener(id==='productSearch'?'input':'change',apply));
    document.getElementById('productFilterReset').addEventListener('click',()=>{state.search='';state.brand='';state.category='';state.stock='';state.sort='name:asc';document.getElementById('productSearch').value='';brand.value='';category.value='';document.getElementById('productStockFilter').value='';document.getElementById('productSort').value='name:asc';renderRows();});
    listenersReady=true;
  }
