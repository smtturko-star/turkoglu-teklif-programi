/* CCTV gerçek model/özellik güncellemesi v3
   Kural: Üretici kaynağıyla doğrulanmayan model numarası ASLA uydurulmaz.
   Doğrulanamayan katalog kaydı model alanı boş bırakılır; açıklamada doğrulama beklediği belirtilir.
*/
(function(){
  const details={
    ...
    'Avenir 6MP HD Kamera':{model:null,description:'Avenir 6 MP HD/AHD için üretici kaynaklarında doğrulanmış bir analog HD model numarası bulunamadı. Yanlış model uydurulmaması için model alanı boş bırakıldı.'},
    'Avenir 8MP HD Kamera':{model:null,description:'Avenir 8 MP HD/AHD için üretici kaynaklarında doğrulanmış bir analog HD model numarası bulunamadı. AV-S242X gerçek 8 MP Wi-Fi IP/PTZ modelidir ve HD analog olarak eşleştirilmedi.'},
    ...
    'Hikvision 4MP HD Kamera':{model:null,description:'Hikvision 4 MP Turbo HD ailesinde birden fazla gerçek model bulunuyor. Tek bir modeli tüm katalog kaydına yanlış atamamak için model alanı boş bırakıldı; gerçek model seçimi bekliyor.'},
    'Hikvision 6MP HD Kamera':{model:null,description:'Hikvision 6 MP Turbo HD için bu katalog kaydına tekil model ataması üretici kaynağıyla ayrıca doğrulanmadan yapılmadı. Model alanı boş bırakıldı.'},
  ...
  async function run(){
    ...
    try{
      ...
      for(const p of (r.data||[])){
        ...
        if(!d)continue;
        if((p.model||null)===d.model && (p.description||'')===d.description)continue;
        const u=await client.from('products').update({model:d.model,description:d.description}).eq('id',p.id);
        if(u.error)throw u.error;
      }
      if(typeof loadAll==='function')await loadAll();
      if(typeof toast==='function')toast('Doğrulanmış CCTV model ve teknik bilgiler uygulandı. Doğrulanamayan modeller boş bırakıldı.');
