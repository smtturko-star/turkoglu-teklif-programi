...
// Giriş ekranı bağlantı kütüphanesi veya ilk başlatma hatası yüzünden sessiz kalmasın.
(function(){
  let booting=false;
  function message(text){const el=document.getElementById('authMsg');if(el)el.textContent=text;}
  function loadSupabase(){
    if(window.supabase?.createClient)return Promise.resolve();
    return new Promise((resolve,reject)=>{
      const s=document.createElement('script');
      s.src='https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2';
      s.onload=()=>window.supabase?.createClient?resolve():reject(new Error('Supabase kütüphanesi yüklenemedi.'));
      s.onerror=()=>reject(new Error('Supabase bağlantı kütüphanesi yüklenemedi.'));
      document.head.appendChild(s);
    });
  }
  async function ensureClient(){
    await loadSupabase();
    if(typeof client==='undefined')throw new Error('Supabase istemcisi başlatılamadı.');
    if(client)return client;
    const cfg=window.__TURKOGLU_SB_CONFIG;
    if(!cfg?.u||!cfg?.k)throw new Error('Supabase bağlantı yapılandırması bulunamadı.');
    client=window.supabase.createClient(cfg.u,cfg.k,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});
    return client;
  }
  window.login=async function(){
    const e=document.getElementById('email')?.value.trim(),p=document.getElementById('password')?.value||'';
    if(!e||!p){message('E-posta ve şifre gerekli.');return;}
    message('Giriş yapılıyor...');
    try{
      const c=await ensureClient();
      const r=await c.auth.signInWithPassword({email:e,password:p});
      if(r.error){message(r.error.message);return;}
      user=r.data.user;
      if(typeof showApp==='function')showApp();
      if(typeof loadAll==='function')await loadAll();
    }catch(err){console.error(err);message(err?.message||'Giriş sırasında bir hata oluştu.');}
  };
  window.signup=async function(){
    const e=document.getElementById('email')?.value.trim(),p=document.getElementById('password')?.value||'';
    if(!e||p.length<6){message('E-posta ve en az 6 karakter şifre gerekli.');return;}
    try{const c=await ensureClient();const r=await c.auth.signUp({email:e,password:p});message(r.error?r.error.message:'Kayıt tamam. E-posta doğrulaması açıksa gelen e-postayı onaylayın.');}
    catch(err){message(err?.message||'Kayıt sırasında bir hata oluştu.');}
  };
  async function recoverBoot(){
    ...
    try{
      ...
      c.auth.onAuthStateChange((event,session)=>{if(session?.user){user=session.user;if(typeof showApp==='function')showApp();setTimeout(()=>typeof loadAll==='function'&&loadAll(),0);}});
    }catch(err){console.error('Auth başlatma:',err);message(err?.message||'Sistem bağlantısı başlatılamadı.');}
    finally{booting=false;}
  }
  setTimeout(recoverBoot,100);
})();
